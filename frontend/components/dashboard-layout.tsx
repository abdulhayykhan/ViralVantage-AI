...
export function DashboardLayout({ children }: Readonly<{ children: ReactNode }>) {
  return (
    <div className="mesh-breathe relative min-h-screen overflow-hidden">
      ...
      <header className="glass-card relative z-10 border-b border-white/10 bg-background/30">
        <div className="mx-auto flex w-full max-w-6xl items-center justify-between px-6 py-4">
          ...
          <div className="glass-card rounded-full px-3 py-1 text-xs text-muted-foreground">Phase 7</div>
        </div>
      </header>

      <main className="relative z-10 mx-auto grid w-full max-w-6xl gap-6 px-6 py-8 lg:grid-cols-[1.2fr_0.8fr]">{children}</main>
    </div>
  );
}
